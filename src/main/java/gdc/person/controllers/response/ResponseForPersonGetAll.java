 ...
 */
@Component
public class ResponseForPersonGetAll extends ResponseGeneratorImpl{

	private static final Logger logger = LoggerFactory.getLogger(ResponseForPersonGetAll.class);
	
	/* (non-Javadoc)
	 * @see gdc.person.controllers.response.ResponseGenerator#generate(gdc.utility.dataservice.DataTransfer, java.util.HashMap)
	 */
	@Override
			...
			if(list != null && list.size()>0) {
				...
				for (Person person : list) {
					...
					if(person.getPersonEmails().size()>0){
						...
						_person.put("person_emails", person_email_list);
					}
					if(person.getPersonContacts().size()>0){
						...
						_person.put("person_contacts", person_contact_list);
					}
					if(person.getPersonAddress().size()>0){
						...
						_person.put("person_addresses", person_address_list);
					}
					person_list.add(_person);
				}
			}
			outputs.put(Key.PERSON_LIST, person_list);
		} catch (Exception e) {
			logger.error("------>> Error ",e);
		}
	}
