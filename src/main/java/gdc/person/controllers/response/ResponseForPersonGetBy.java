 ...
 */
@Component
public class ResponseForPersonGetBy extends ResponseGeneratorImpl {

	private static final Logger logger = LoggerFactory.getLogger(ResponseForPersonGetBy.class);
	...
	private void processOutputs(DataTransfer dataTrans, HashMap<String, Object> outputs) {
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Person person = (Person) dataTrans.getOutput("person");
			HashMap<String, Object> _person = null;
			if(person!=null) {
				_person = new HashMap<String, Object>();
